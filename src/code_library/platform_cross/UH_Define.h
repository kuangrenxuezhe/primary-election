...
#if defined(CL_UCMD_ENABLE) && defined(_LINUX_ENV_)
#include "UC_Memory_Detection.h"
#endif
...
static inline ssize_t cp_dio_write(var_4 fildes, const var_vd* buf, size_t nbyte)
{
	return write(fildes, buf, nbyte);
}

static inline ssize_t cp_dio_read(var_4 fildes, var_vd* buf, size_t nbyte)
{
...
static var_4 cp_dir_travel_folder(var_vd* handle, var_1* file_name)
{
    struct dirent* dirent;
...
public:
	...
	var_4 cat_buf(T_Type* value, var_8 size = 1)
	{
		...
		if(n_size > m_size)
			t_size = (n_size + m_base - 1) / m_base * m_base;
	...
	var_4 set_buf(T_Type* value, var_8 size = 1)
	{
		...
		if(size > m_size)
			t_size = (size + m_base - 1) / m_base * m_base;
...
static var_4 cp_set_overtime(CP_SOCKET_T in_socket, var_4 in_time_ms)
{
#ifdef _WIN32_ENV_
...
static var_4 cp_sendfile(CP_SOCKET_T in_socket, var_1* in_file)
{
	...
	if(cp_sendbuf(in_socket, (var_1*)&file_len, 8))
	{
...
static var_4 cp_create_thread(void* (in_function)(void*), void* in_argv, var_4 create_num)
{
	pthread_t pid;
...
const var_vd cp_random_shuffle(T_Key* result, var_4 num)
{
