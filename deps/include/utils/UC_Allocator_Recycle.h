...
public:
	...
	var_4 initMem(var_8 lBlockSize/* 每块buf大小 */, var_8 lMaxBlockNum = 0/* 最大buf块数 */, var_8 lPerAllocBlockNum = 0/* 每段"buffer段"buf块数 */)
	{
		...
		{
            if(lMaxBlockNum < DEF_PRE_BUF_BLK_NUM)
                lPerAllocBlockNum = lMaxBlockNum;
            else
                lPerAllocBlockNum = DEF_PRE_BUF_BLK_NUM;
            
            m_lMaxBufNum = (lMaxBlockNum + lPerAllocBlockNum - 1) / lPerAllocBlockNum; // 最大分配"buffer段"数量
		}
	...
	var_1* AllocMem()
	{
		...
		if(m_lMaxBlockNum && m_lStat_AllocNum >= m_lMaxBlockNum)
		{
			m_cMutexLock.unlock();
			return NULL;
		}
