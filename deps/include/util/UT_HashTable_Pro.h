...
public:
	...
	var_4 travel_key(var_vd* handle, T_Key& key, var_vd** const reference = NULL)
	{
		...
		var_8  travel_idx = ti->travel_idx;
    ...
    var_4 save_quick(var_1* library_file, var_4 is_lock = 1)
	{
		...
		try {
            ...
            for(var_8 i = 0; i< m_table_size; i++)
            {
                ...
                {
                    if(fwrite(ptr, sizeof(T_Key), 1, fp) != 1)
                        throw -5;
		...
		try
		{
            ...
            for(var_8 i = 0; i < table_size; i++)
            {
                ...
                for(var_4 j = 0; j < num; j++)
                {
                    ...
                    if(fread(node, sizeof(T_Key), 1, fp) != 1)
                        throw -8;
	...
	var_4 restore()
	{
		...
		if(access(m_name_inc, 0) == 0)
		{
			...
			while(cur_size < all_size)
			{
				...
				{
					if(m_store_size && fread(buf + 8 + sizeof(T_Key), m_store_size, 1, m_file_inc) != 1)
						break;
	...
	var_4 persistent(T_Key key, var_vd* value, var_4 is_delete)
	{
		...
		try
		{
			...
			if(fwrite(&key, sizeof(T_Key), 1, m_file_inc) != 1)
				throw -2;
			if(is_delete == 0 && m_store_size && fwrite(value, m_store_size, 1, m_file_inc) != 1)
				throw -3;
